...
<head>
	...
	<meta charset="utf-8">
	<title>PromoCash - Coupon Consumati</title>
...
<body>
	...
	<h1 style="display:inline">Utilizzo coupons di: &nbsp;<?php	if (isset($_SESSION['user']))
                        ...
                        else//Se è da considerare solo un merchant 
                        {
                            if (isset($_POST['btn_filtraCoupon'])){//Se ho premuto il pulsante Filtra
                                if(isset($_POST['selectMerchantCode'])&&isset($_POST['selectPhoneService'])){
                                    ...
                                    try{
                                        $NewCoupon	= GetConsumedCouopon($SelectedMerchant,$SelectedPhoneService,$Coupon_Channel,$StartDate,$StartTime,$StopDate,$StopTime,"-23",true);
                                    }
                                    catch (Exception $ex)
                                    {
                                        logWrite("error","Eccezione nella chiamata GetConsumedCouopon.Dettagli:".$ex->getMessage());
                            ...
                            {
                                try{
                                    $NewCoupon	= GetConsumedCouopon($sxml_PhoneService->PhoneService[0]->Merchant_code,$sxml_PhoneService->PhoneService[0]->Phone_service,
